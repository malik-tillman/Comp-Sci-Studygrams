...
def mult_matrix(m_plier, m_plicand):
    ...
    dm = [[0] * sz for _ in range(sz)]

    for r in range(len(m_plier)):  # for(int i = 0; i <m.len(); i++
        ...
        for c in range(len(m_plicand[0])):
            ...
            for r2 in range(len(m_plicand)):
                ...
                dm[r][c] += m_plier[r][r2] * m_plicand[r2][c]  # Dot Product Formula for 2 x 2 Matrix Multiplication
